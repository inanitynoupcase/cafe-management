...
import { deleteCustomerAccount, getCustomerAccount } from '../redux/apiRequest';
...
export default function CustomerAccount() {
  ...
  const [filteredUsers, totalUsers] = applySortFilter(customerAccountList, getComparator(order, orderBy), filterName);
  ...
  const handleNew = () => {
    navigate('/dashboard/customeraccount/new');
    // console.log(navigate('/edit'));
  };

  const handleEdit = (event, name) => {
    navigate(`/dashboard/customeraccount/edit/${name}`);
  };

  const [isDeleting, setIsDeleting] = useState(false);
  const [deletedCustomerAccount, setDeletedCustomerAccount] = useState(null);

  const handleDelete = async (event, idTaiKhoan) => {
    setIsDeleting(true);
    try {
      // Gọi hàm xóa nhân viên và lấy kết quả trạng thái của request (status code)
      const statusCode = await deleteCustomerAccount(user.accessToken, dispatch, idTaiKhoan, axiosJWT);

      // Nếu xóa thành công (statusCode === 200), cập nhật state deletedStaffInfo để tải lại dữ liệu
      if (statusCode === 200) {
        setDeletedCustomerAccount(idTaiKhoan);
        setNotification({ open: true, message: 'Xóa tài khoản thành công', severity: 'success' });
      }
    } catch (err) {
      console.log('Lỗi xóa tài khoản nhân viên:', err);
      setNotification({ open: true, message: `Lỗi xóa tài khoản khách hàng `, severity: 'error' });
    }
  ...
  useEffect(() => {
    if (deletedCustomerAccount !== null) {
      ...
      setDeletedCustomerAccount(null);
    }
  ...
  return (
    <>
      ...
      <Helmet>
        <title> Customer Account | The Coffee </title>
      </Helmet>
      {user.ID_ChucVu === 3 ? (
        <Container>
                  ...
                  <TableRow>
                    <TableCell align="center" colSpan={3}>
                      <Typography variant="body1" align="center">
                        Bạn không có quyền truy cập vào tài khoản khách hàng.
                      </Typography>
                    </TableCell>
                  </TableRow>
                </TableBody>
                ...
                <Table>
                  ...
                  <TableBody>
                    {filteredUsers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                      const { idTaiKhoan, TenTaiKhoan, Hoten, trangThai } = row;
