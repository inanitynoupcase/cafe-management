...
import { updateCustomerInfo, getCustomerInfo } from '../../redux/apiRequest';
...
const EditAccountInfo = () => {
  ...
  const handleBack = () => {
    navigate('/dashboard/customerinfo');
  };
  ...
  const needEdit = test.find((khachHang) => khachHang.idKhachHang === parseInt(name, 10));
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      updateCustomerInfo(user.accessToken, dispatch, editUser, axiosJWT)
        .then((statusCode) => {
          ...
          if (statusCode === 200) {
            ...
            setNotification({ open: true, message: `Lỗi: Email đã tồn tại`, severity: 'error' });
          }
        })
  ...
  return (
    <>
      ...
      <Helmet>
        <title> Customer Account Edit | The Coffee </title>
      </Helmet>
