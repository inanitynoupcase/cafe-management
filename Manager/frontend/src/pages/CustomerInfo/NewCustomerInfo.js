...
import { createCustomerInfo, getCustomerInfo } from '../../redux/apiRequest';
...
const NewCustomerInfo = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, { resetForm }) => {
      ...
      console.log(newUser);
      createCustomerInfo(user.accessToken, dispatch, newUser, axiosJWT)
        .then((statusCode) => {
          console.log('statusCode: ', statusCode);
          if (statusCode === 201) {
            getCustomerInfo(user.accessToken, dispatch, axiosJWT);
            setNotification({ open: true, message: 'Tạo mới khách hàng thành công', severity: 'success' });
            resetForm();
          } else {
  ...
  return (
    <>
      ...
      <Helmet>
        <title> New Customer Info | The Coffee </title>
      </Helmet>
