...
import { deleteCustomerInfo, getCustomerInfo } from '../redux/apiRequest';
...
const TABLE_HEAD = [
  { id: 'idKhachHang', label: 'ID Khách Hàng', alignRight: false },
...
export default function CustomerInfo() {
  ...
  const [orderBy, setOrderBy] = useState('idKhachHang');

  const [filterName, setFilterName] = useState('');

  const [rowsPerPage, setRowsPerPage] = useState(5);

  const [filteredUsers, totalUsers] = applySortFilter(customerInfoList, getComparator(order, orderBy), filterName);
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = filteredUsers.map((n) => n.idKhachHang);
  ...
  const handleNew = () => {
    navigate('/dashboard/customerinfo/new');
    // console.log(navigate('/edit'));
  };

  const handleEdit = (event, name) => {
    navigate(`/dashboard/customerinfo/edit/${name}`);

    const [test, test1] = customerInfoList;
    console.log(test);
    const needEdit = test.find((nhanVien) => nhanVien.idKhachHang === name);
  };
  const [isDeleting, setIsDeleting] = useState(false);
  const [deletedCustomerInfo, setDeletedCustomerInfo] = useState(null);

  const handleDelete = async (event, idKhachHang) => {
    setIsDeleting(true);
    try {
      // Gọi hàm xóa nhân viên và lấy kết quả trạng thái của request (status code)
      const statusCode = await deleteCustomerInfo(user.accessToken, dispatch, idKhachHang, axiosJWT);

      // Nếu xóa thành công (statusCode === 200), cập nhật state deletedStaffInfo để tải lại dữ liệu
      if (statusCode === 200) {
        setDeletedCustomerInfo(idKhachHang);
        setNotification({ open: true, message: 'Xóa Khách Hàng thành công', severity: 'success' });
      }
    } catch (err) {
      console.log('Lỗi xóa nhân viên:', err);
      setNotification({ open: true, message: `Lỗi xóa Khách Hàng`, severity: 'error' });
    }
  ...
  return (
    <>
      ...
      <Helmet>
        <title> Customer Information | The Coffee </title>
      </Helmet>
      {user.ID_ChucVu === 3 ? (
        <Container>
                ...
                <Table>
                  ...
                  <TableBody>
                    {filteredUsers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                      const { idKhachHang, HoTen, NgaySinh, GioiTinh, SDT, Email } = row;
                      const selectedUser = selected.indexOf(idKhachHang) !== -1;

                      return (
                        <TableRow hover key={idKhachHang} tabIndex={-1} role="checkbox" selected={selectedUser}>
                          <TableCell padding="checkbox">
                            <Checkbox checked={selectedUser} onChange={(event) => handleClick(event, idKhachHang)} />
                          </TableCell>
                          ...
                          <TableCell align="right">
                            <IconButton
                              size="large"
                              color="warning"
                              onClick={(event) => handleEdit(event, idKhachHang)}
                            >
                            ...
                            <IconButton
                              size="large"
                              color="error"
                              onClick={(event) => handleDelete(event, idKhachHang)}
                            >
